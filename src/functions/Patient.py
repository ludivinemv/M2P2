...
class Patient:
    ...
    def __init__(self, ref, PATH_TO_DATA):
        ...
        self.correct=correction(ref)

# --------------------------
# Set of functions used to pre process dcm slice
# --------------------------
def correction(ref):
    data = pd.read_csv('corrections.csv',sep='\t')
...
#    number=[]
#    for i in range(size):
#        number.append(dcmImage[i][0x00200013].value)
##    print('number',number)
...
#        image = sitk.GetArrayFromImage(image3D)[:,:,249]
#        plt.imshow(image)
        image= sitk.GetArrayFromImage(image3D)
        print("   Get Array done")        

        TiffPath = os.path.join('patient.tif')
        print("   TiffPath done")   
        tifffile.imsave(TiffPath, image)
        return image3D
#        return rescaledImage
