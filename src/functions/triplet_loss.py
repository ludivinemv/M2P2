...
def batch_hard_triplet_loss(margin=0.001, squared=False):
    ...
    def loss(y_true,y_pred):
        ...
        indices_equal = tf.cast(tf.eye(tf.shape(labels)[0]), tf.bool)
        indices_not_equal = tf.logical_not(indices_equal)
        find_anchor_negative =  tf.reduce_max(pairwise_dist, reduction_indices=[1])
      
        
        sub = pairwise_dist + 10000*tf.eye(tf.shape(pairwise_dist)[0])
        find_anchor_positive =  tf.reduce_min(sub, reduction_indices=[1])
        fn = tf.where(tf.equal(pairwise_dist,find_anchor_positive),tf.bool)
        # =tf.map_fn(fn, elems)
         
         
         
        mask_anchor_positive = tf.where(tf.equal(find_anchor_positive,))
